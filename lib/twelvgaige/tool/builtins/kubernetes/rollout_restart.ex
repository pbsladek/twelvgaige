defmodule Twelvgaige.Tool.Builtins.Kubernetes.RolloutRestart do
  @moduledoc """
  Restart a namespaced Kubernetes workload through structured `kubectl rollout restart` argv.
  """
  ...
  @resources ~w(deployments statefulsets daemonsets)

  @impl true
  def name, do: "kubectl_rollout_restart"

  @impl true
  def description, do: "Restart a namespaced Kubernetes workload rollout."
  ...
  def execute(input, opts) do
    with :ok <- Common.require_confirm(input, name()),
      ...
      {:ok,
       Common.output_base(target, "rollout_restart", result.duration_ms)
       |> Map.merge(%{
  ...
  defp args(target) do
    Common.base_args(target) ++
      ["rollout", "restart", "#{target.resource}/#{target.name}"]
  end
