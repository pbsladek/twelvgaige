...
- `[~]` Build and smoke-test Burrito binaries on supported runners with Zig `0.15.2`, `xz`, and `7z`/`7zz` for Windows targets. macOS Apple Silicon host smoke is verified locally with Homebrew `zig@0.15` and `BURRITO_CUSTOM_ERTS_MACOS_SILICON`; Linux, Linux ARM64, Intel macOS, and Windows executable artifacts still need runner-native smoke coverage.
